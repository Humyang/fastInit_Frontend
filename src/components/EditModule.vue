<template>
  <div class="EditModule">
    <div class="floder" id="floder">
        <div id="moduleTree" class="tree_block">
        
        </div>
        <slide_line v-on:offsetX="floder_move" parentId="floder"/>
    </div>
        <!-- 代码编辑 -->
...
import slide_line from "../common/slide_line";
export default {
  components: {
    editor,
    slide_line
  ...
  methods: {
    ...
    saveNodeData: function(event) {
      let self = this;
      API.MODULE
        .saveNodeData(event.patch_list, self.selectedNodeId)
        .then(function(res) {
